...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.GET_FILTERED, query = "SELECT m FROM Meal m WHERE m.user.id=:user_id AND m.dateTime >= :date_start AND m.dateTime < :date_end ORDER BY m.dateTime DESC")}
)
@Entity
